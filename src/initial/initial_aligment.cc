...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        ...
        tmp_A = frame_j->second.pre_integration->jacobian_enc.template block<3, 3>(O_R, O_BG);
...
bool LinearAlignmentWithS(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x, double &s_)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        ...
        A.block<6, 6>(i * 3, i * 3) += r_A.topLeftCorner<6, 6>();
        b.segment<6>(i * 3) += r_b.head<6>();
        A.bottomRightCorner<4, 3>() += r_A.bottomRightCorner<4, 3>();
        b.tail<3>() += r_b.tail<3>();
        A.block<6, 3>(i * 3, n_state - 3) += r_A.topRightCorner<6, 3>();
        A.block<4, 6>(n_state - 4, i * 3) += r_A.bottomLeftCorner<4, 6>();
    ...
    RefineGravityWithS(all_image_frame, g, x,s_);
    kv = -1;
    for (frame_i = all_image_frame.begin(); frame_i != all_image_frame.end(); frame_i++)
    {
            kv++;
...
{
    // cout << "VisualIMUAlignment()" << endl;
    solveGyroscopeBias(all_image_frame, Bgs);
    double s;
    solve_s(all_image_frame,s);
    if(LinearAlignmentWithS(all_image_frame, g, x, s))
        return true;
    else 
        return false;
}
